    ...
    internal static class Commander {
        ...
        internal static void Scan() {
            if (pState == ProgramStates.Ready) {
                ...
                measureMode.GraphUpdateDelegate = (p, peak) => { Graph.Instance.updateGraphDuringScanMeasure(Device.Detector1, Device.Detector2, p); };
        ...
        internal static bool Sense() {
            if (pState == ProgramStates.Ready) {
                if (SomePointsUsed) {
                    ...
                    measureMode.GraphUpdateDelegate = (p, peak) => { Graph.Instance.updateGraphDuringPreciseMeasure(p, peak); };
                    initMeasure(ProgramStates.Measure);
                    return true;
                } else {
        ...
        private static void NewBackgroundMeasureReady(int[] recreate) {
        ...
        private static void NewMonitorMeasureReady(int[] recreate) {
